...
class TissueTestAnno(Dataset): #继承Dataset
    ...
    def __init__(self, img_dir, rois, mask, page = 0 ,mask_page = 6,transform=None,stride = 512, grid_size = 1024): #__init__是初始化该类的一些基础参数
        ...
        for bbox in self.ROIs:
            new_anchors = bbox_divide(bbox,stride,grid_size,self.mask,self.scale,self.height,self.width)
            anchors += new_anchors
            anchor_group += [anchor_idx] * len(new_anchors)
            if len(new_anchors) > 0:
                anchor_idx += 1

        # double check out of array error, may be redundant 
        for anchor, anchor_idx in zip(anchors, anchor_group):
            if anchor[0] + self.grid_size < self.width - 1 and anchor[1] + self.grid_size < self.height - 1:
                self.anchors.append(anchor)
                self.anchor_group.append(anchor_idx)
    ...
    def get_anchors_normalized(self):
        res = np.array(self.anchors).astype(np.float32)
        res[:, 0] = res[:,0] / self.width
        res[:, 1] = res[:,1] / self.height
        return res
    
    def get_anchor_group(self):
